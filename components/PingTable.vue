    ...
    export default {
        ...
        computed: {
            ...mapState({
				...
				pingUpdateTable: 'pingUpdateTable',
            }),
		},
		beforeDestroy() {
		},
		...
		watch:{
			"watchingAddr": function(){
				this.updateTransactionList();
			},
			"$store.state.balance": function(){
				this.updateTransactionList();
			},
			"pingUpdateTable": function(){
				this.updateTransactionList();
			},
		},
        methods: {
			...
			<div class="ping-list list-item-wrap" v-for="tx in transactionList" v-if="isPingTx(tx)" @click="onMessageClick(tx)">
				<div class="list-item list-item--tappable" @click.stop="onProfileClick(tx)" >
					<div class="list-item__left">
						<img class="list-item__thumbnail" :src="minterscanProfilesList[tx.from]&&minterscanProfilesList[tx.from].icon?getMinterscanIconUrl(tx.from):getAvatarUrl(tx.from)" alt="" role="presentation">
					</div>
